 ...
 * @author Yvonne Wang
 */
class ComponentDriver_rightClick_Test extends ComponentDriver_TestCase {
  private ClickRecorderManager clickRecorder = new ClickRecorderManager();

  @Test
  void should_Right_Click_Component() {
    showWindow();
  ...
  @Test
  void should_Right_Click_Disabled_Component() {
    showWindow();
    disableButton();
    ClickRecorder recorder = clickRecorder.attachDirectlyTo(window.button);
    driver.rightClick(window.button);
  ...
  void should_Throw_Error_If_Component_Is_Disabled_And_ClickOnDisabledAllowd_Is_False() {
    ...
    disableButton();
    try {
      ExpectedException.assertIllegalStateIsDisabledComponent(() -> driver.rightClick(window.button));
    } finally {
  ...
  void should_Throw_Error_If_Component_Is_Not_Showing_On_The_Screen() {
    ClickRecorder recorder = clickRecorder.attachDirectlyTo(window.button);
    try {
      ExpectedException.assertIllegalStateIsNotShowingComponent(() -> driver.rightClick(window.button));
    } finally {
      recorder.wasNotClicked();
    }
  }
